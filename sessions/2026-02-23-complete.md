...
Architecture docs now live in a git repo (`~/getinsync-architecture`). `MANIFEST.md` in the repo root is the single source of truth — git tracks version history, so versioned-filename copies (e.g. `gis-architecture-manifest-v1_25.md`) are retired.

Stuart keeps ~9 key files synced to the Claude Opus project manually. The file list and sync rules are documented in the new "Document Sources — Two Environments" section of MANIFEST.md. When project files conflict with the repo, the repo wins.
