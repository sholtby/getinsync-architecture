...
| ~~86~~ | ~~Data Quality~~ | ~~Legacy title-case `deployment_profiles.paid_action`~~ | ✅ **CLOSED Apr 18.** Case normalised to lowercase as part of the broader PAID framework terminology cleanup (branch `fix/paid-framework-terminology`, v2026.4.12). CHECK constraint tightened from 10 permissive values to the canonical 4 (`plan/address/ignore/delay`) — TitleCase and the non-canonical `improve`/`divest` variants are no longer accepted. See "Completed Apr 18" row. | -- | Stuart |
...
| 100 | Feature | Garland Claim Audit — parked gap backlog (S/M/L) | 9 session prompts written but parked. Presentation fixed via language in audit response (`planning/GarlandClaims/garland-presentation-audit-response.md`). XS-1 (ai-chat-owner-filter) shipped v2026.4.11. Prompts in `planning/GarlandClaims/`. Features are real product gaps (double-count guard, contract notifications, budget trend chart, restricted role, steward role, budget AI tool, lifecycle auto-lookup, SOC2 evidence automation) but not MVP blockers. | -- | Stuart + Claude Code |
...
| Item | Resolution |
|------|------------|
| PAID framework terminology cleanup | ✅ COMPLETE. Shipped in v2026.4.12 on branch `fix/paid-framework-terminology` (merged dev + main, Edge Functions deployed). Closes #86 and addresses a broader bug class: the wrong labels `improve` (should be Ignore) and `divest` (should be Delay) had leaked into the DB CHECK constraint, 3 aggregation views, 7 frontend files, both AI chat Edge Functions, the Garland marketing deck, and the data-quality validation script. Compensating dual-label CASE expressions in views and frontend hooks had cemented the bug. **Remediation landed in 5 layers:** (1) SQL chunk `schema/2026-04-18-paid-framework-cleanup.sql` — lowercased paid_action data, replaced the 10-value CHECK constraint with the canonical 4 (`plan/address/ignore/delay`), renamed `improve_count → ignore_count` and `divest_count → delay_count` on `vw_dashboard_summary`, `vw_dashboard_summary_scoped`, `vw_dashboard_workspace_breakdown` (ALTER VIEW ... RENAME COLUMN first, then CREATE OR REPLACE — Postgres rejects rename-via-replace). (2) Frontend: `view-contracts.ts`, `usePortfolios.ts`, `useApplications.ts`, `DashboardPage.tsx`, `useDashboardSummary.ts`, `AssessmentCompletionBar.tsx`, `DerivedScoresTab.tsx` — column renames, dual-mapping branches deleted, helper text corrected. (3) Edge Functions: `ai-chat/tools.ts` enum switched to lowercase to match DB after normalisation (was TitleCase → would have broken `.eq('paid_action', value)` post-normalisation), `ai-chat/system-prompt.ts` legacy-label workaround comment removed, `apm-chat/index.ts` corrected a separate wrong variant ("Protect/Advance/Innovate/Divest" — not even the same bug). (4) Architecture docs: `marketing/GetInSync_NextGen_Garland_18Slide_Final.md` Slides 8 & 9 fixed, `testing/data-quality-validation.sql` CHECK 7 tightened. (5) Memory: `feedback_paid_terminology.md` updated with remediation note, `debugging.md` entry added. **Platform-wide verification:** all 6 namespaces show only canonical lowercase values; zero DB functions/views/triggers reference old labels; 0 rows in the DB ever held `improve`/`divest`, so no data migration — this was a pure constraint + code + docs alignment + case normalisation. Version bumped 2026.4.11 → 2026.4.12. |
...
| Item | Resolution |
|------|------------|
| Garland Presentation Claim Audit — slide language & talking points | ✅ COMPLETE. 11-slide City of Garland presentation audited claim-by-claim against codebase and schema. 3 red flags, 10 yellow flags identified. All resolved via language adjustments in `planning/GarlandClaims/garland-presentation-audit-response.md` — no code changes needed for presentation accuracy. 9 session prompts (S/M/L gaps) written but PARKED as non-MVP backlog (#100). XL/2XL items tracked in product roadmap (Phases 37, 40, 43, 51). |
...
| **MEDIUM** | 25 | Identity rewrite, compliance (3 more OVERDUE), Delta enablement, demo data, website, RBAC assessment split, scope indicator, **multi-server DPs (#93)**, **integration field parity (#94)**, namespace UI, budget alerts, assessment tour, **Garland parked backlog (#100)** |
...
| **Total Open** | 43 | Apr 17: +1 Garland parked backlog (#100). Apr 12: +2 features (#93 multi-server DP, #94 integration field parity). #63 superseded by #93. |
