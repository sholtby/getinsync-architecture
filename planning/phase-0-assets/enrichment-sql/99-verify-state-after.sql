...
-- Editor: Pure SQL — no psql backslash meta-commands. Safe to paste into
...
-- --- Chunk 01 result: contacts on showcase app (expect 3 rows) ---
SELECT
  '01 result — CAD showcase app fields' AS section,
...
SELECT
  '01 result — CAD application contacts detail' AS section,
  ac.role_type,
  c.display_name,
  c.email,
  ac.is_primary
...
SELECT
  '01 result — contacts in Riverside namespace' AS section,
  count(*) AS riverside_contacts_total_after
FROM contacts
WHERE namespace_id = 'a1b2c3d4-e5f6-7890-abcd-ef1234567890';

-- --- Chunk 02 result: integrations DP alignment (expect 3 aligned, 0 unnamed) ---
SELECT
  '02 result — integrations DP alignment counts' AS section,
...
SELECT
  '02 result — integrations listing' AS section,
  ai.name,
...
-- --- Chunk 03 result: FY2026 workspace budgets (expect 5 rows) ---
SELECT
  '03 result — FY2026 workspace budgets' AS section,
  w.name,
  wb.fiscal_year,
  wb.budget_amount,
  wb.is_current
...
-- --- Chunk 04 result: IT services contract data (expect 3 with contract) ---
SELECT
  '04 result — IT services contract coverage' AS section,
...
SELECT
  '04 result — IT services with contract data' AS section,
  name,
  annual_cost,
  contract_reference,
  contract_start_date,
...
-- --- Chunk 05 result (OPTIONAL): CAD PROD DP ops fields ---
SELECT
  '05 result — CAD PROD DP ops fields' AS section,
  dp.id,
  dp.name,
  dp.data_center_id,
...
-- --- Chunk 06 result: cost_bundle DPs on showcase apps (expect 3 rows) ---
SELECT
  '06 result — cost bundle DP listing' AS section,
  a.name          AS app_name,
...
SELECT
  '06 result — recurring bundle total per showcase app' AS section,
  a.name                                                                              AS app_name,
...
SELECT
  '06 result — vw_deployment_profile_costs for showcase apps' AS section,
  application_name,
...
-- AFTER snapshot complete. Compare against BEFORE snapshot to confirm.
