...
| identity-security/soc2-evidence-collection.md | v1.3 | 🟢 | SOC2 monthly evidence collection — 52 audited tables, 106 tables, 402 RLS policies (updated Apr 12). EV-002 collected. |
...
| testing/security-posture-validation.sql | v1.6 | 🟢 | Standalone security validation — PASS/FAIL output for all 106 tables + 42 views (incl. view GRANTs + platform admin bypass check) |
...
| Document | Version | Status | Description |
|----------|---------|--------|-------------|
| features/csv-import/architecture.md | v2.0 | 🟢 | **CSV Import v2 — DEPLOYED. Self-serve onboarding: 5-step wizard (target, upload, preview, import, complete). 14 CSV columns (app fields + DP fields + assessment scores). Import batch tracking + undo with modification detection. Namespace admin only. 500-row cap. Template download. Import history table.** |
...
| planning/tech-health-refactoring-plan.md | v1.0 | 🟡 | **Refactoring plan: decompose TechnologyHealthSummary (1563 lines) and TechnologyHealthByApplication (1011 lines) into hooks/components/utils. Two parallel sessions, prompt-ready.** |
...
| Document | Version | Status | Description |
|----------|---------|--------|-------------|
| features/application-profile/schema-mapping.md | v1.1 | 🟡 | **Application Profile — canonical one-page business-language summary; atomic unit of the platform.** 11-block proposal (Identity, Business Purpose, User Community, Information Domains, Ownership, Criticality, Lifecycle, Context, Cost, Tech Debt & Remediation, Assessment) mapped field-by-field against existing schema. Identifies 7 NEW-STORED columns, 3 new tables (capabilities, data_domain_types, application_tech_debt), 6 NEW-AI narrative fields with cache strategy. Recommends VIEW (`vw_application_profile`) + new `application_narrative_cache` table. Aligns the profile to be the single shape the Publish Assessment RPC consumes. |
| planning/application-profile/prompts/ | v1.0 | 🟡 | **Paste-ready Claude Code session prompts (8 files).** One atomic prompt per session (1, 2, 3, 4, 5, 5b, 6) plus a README. Each prompt is self-contained — fresh Claude Code session pastes the file content and has everything needed to execute: required reads, rules, concrete changes, exit criteria, commit guidance. Implements `session-plan.md` v1.2. |
| planning/application-profile/session-plan.md | v1.2 | 🟡 | **Tier 1 session plan (6 sessions + optional 5b, ~13–15 hrs).** Breaks Application Profile Tier 1 into six serial sessions: schema migration + data cleanup, `vw_application_profile` view, TypeScript interfaces, hooks, `ApplicationDetailDrawer` evolution, doc alignment + publish-assessment RPC reshape. Cross-references open-items matrix — bundles #86, #87, #94 Phase 1 schema into Session 1; #97 (optional #96) into Session 3. No merge-conflict risk (prerequisites #93 and #67 already on main). v1.1 amendments: (a) `application_categories` pulled into Tier 1 — Session 2 adds `category_names jsonb` to the view, Session 5 renders category chips in Block 2 Business Purpose (capabilities remain Tier 2 — different concept); (b) Session 5 subcomponent extraction is now mandatory with block components under `src/components/applications/profile/`, drawer becomes a thin orchestrator. v1.2 amendments: (c) Plan-status tracking added — four new columns on `portfolio_assignments` (`has_plan` tri-state, `plan_note`, `plan_document_url`, `planned_remediation_date`) capture workshop-surfaced information; Session 1 adds columns, Session 2 projects them in the view, Session 3 adds them to the TS interface, Session 5 renders a "Response Plan" sub-section in Block 7 (Lifecycle Position) with inline edit; new optional Session 5b adds a post-completion capture panel to `PortfolioAssessmentWizard.tsx` (deferrable to Tier 1.5); Session 6 adds EA Handoff auto-finding patterns to the Publish Assessment spec. Plan status does NOT affect PAID derivation — it contextualizes PAID. Tier 1 still excludes Tier 2 concept tables (capabilities, data domains, tech debt items) and narrative generation pipeline. |
...
| v2.16 | 2026-04-16 | **Application Profile session plan v1.0 → v1.1 amendments.** `planning/application-profile/session-plan.md` — (a) `application_categories` pulled into Tier 1: Session 2 adds `category_names jsonb` aggregate to `vw_application_profile` (pattern from [DashboardPage.tsx:79](src/components/dashboard/DashboardPage.tsx:79) — `application_category_assignments` junction with nested `category:application_categories(...)` select); Session 3 adds the field to `VwApplicationProfile` TS interface; Session 5 renders category tag chips in Block 2 Business Purpose. Capabilities remain Tier 2 — different concept (categories = what type of software; capabilities = what business function it enables). (b) Session 5 subcomponent extraction now **mandatory** — new `src/components/applications/profile/` directory with one block component per section (`IdentityBlock`, `BusinessPurposeBlock`, etc.); `ApplicationDetailDrawer.tsx` becomes a thin orchestrator (<400 lines). Eleven blocks in one file would cross the 800-line threshold; extract up front. |
| v2.15 | 2026-04-16 | **NEW: Application Profile Tier 1 session plan.** `planning/application-profile/session-plan.md` v1.0 🟡 — 6-session serial plan (~12–14 hrs total) for shipping AP Tier 1: Session 1 schema migration + data cleanup (bundles #86 paid_action title-case normalization, #87 business_assessment_status normalization, #94 Phase 1 integration schema additions), Session 2 `vw_application_profile` view, Session 3 TypeScript + view-contract cleanup (bundles #97, optionally #96), Session 4 hooks, Session 5 `ApplicationDetailDrawer` evolution, Session 6 doc alignment + publish-assessment RPC reshape. Section 2 cross-references every open-item touching `applications`/`application_integrations`/`application_contacts`/`deployment_profiles` schemas. Section 3 confirms no merge-conflict risk (#93 multi-server + #67 integration-DP already on main). Section 4 notes the publish-assessment RPC spec will be reshaped in Session 6 to return `vw_application_profile` per-app + workspace aggregates. Tier 2 explicitly deferred: capability model, data-domains taxonomy, `application_tech_debt` table, narrative generation pipeline, cost_trend time-series. |
| v2.14 | 2026-04-16 | **PAID terminology canonicalized.** Stuart confirmed PAID = Plan/Address/Delay/Ignore only. Never Improve/Divest (non-canonical — from other APM frameworks). Corrected 6 doc locations: `features/application-profile/schema-mapping.md` (Block 7 note rewritten), `features/publish-assessment/architecture.md` (§System Prompt Strategy Tolerate+Divest example reframed as Tolerate+Address; §Step 2 PAID parenthetical), `catalogs/csdm-application-attributes.md` (attribute table), `marketing/positioning-statements.md` (PAID framework description), `csdm-crawl-toolkit/skills/csdm-crawl/references/getinsync-bridge.md` (§TIME/PAID framework), `planning/ai-chat-harness-optimization/01-session-prompt-harness-code.md` (tool schema enum + description). Rule applies to all docs, UI labels, narratives, and AI prompts (especially publish-assessment Edge Function). **Code-repo follow-ups flagged (not touched in this commit):** `src/components/DerivedScoresTab.tsx:208` user-facing text, `supabase/functions/ai-chat/tools.ts:119-120` tool schema enum, `supabase/functions/ai-chat/tools.ts:269-270` output formatting, `supabase/functions/ai-chat/system-prompt.ts:178` legacy-mapping comment, `supabase/functions/apm-chat/index.ts:207` wrong PAID labels ("Protect/Advance/Innovate/Divest"), `vw_dashboard_summary.improve_count/divest_count` column names, `deployment_profiles_paid_action_check` currently tolerates all 6 values — needs tightening to 4. |
...
| v2.00 | 2026-04-07 | **CSDM Cleanup (Chunk 5).** IT Service Catalog cleanup: removed per-app links from overhead services (Network Infrastructure, Cybersecurity Ops), reclassified ITSM Platform → business app "ServiceNow ITSM" ($85K cost bundle), moved M365 Enterprise + Collaboration & Conferencing to Platform > Runtime/PaaS. IT Service Catalog Name column widened (col-span-4→5). `guides/whats-new.md` updated. |
...
| v1.99 | 2026-04-08 | **COR Roadmap Demo Data Restore.** Roadmap data lost during cor-demo-data-reset Phase 1 but omitted from Phase 2. NEW: `schema/cor-demo-data-roadmap-seed.sql` — Phase 3 seed (7 contacts, 8 findings, 6 initiatives, 6 ideas, 2 programs, 4 dependencies, 9 DP links, 6 IT service links). NEW: `schema/cor-demo-data-sirsidynix-app.sql` — supplemental SirsiDynix Symphony app + DP + software product. Updated: `features/roadmap/architecture.md` (seed data status), `planning/cor-demo-data-reset-implementation-plan.md` (Phase 3 added). Orphan cleanup: 2 stale DPs + 5 stale programs removed. Document count 120→120 (SQL scripts, no new docs). |
...
| v1.95 | 2026-04-13 | NEW: `planning/tech-health-refactoring-plan.md` v1.0 🟡 — Decomposition plan for TechnologyHealthSummary (1563 lines) and TechnologyHealthByApplication (1011 lines). Two parallel sessions with extraction steps, verification checklists, and ready-to-use prompts. Target: ~400 and ~350 lines respectively via hooks/components/utils extraction. |
