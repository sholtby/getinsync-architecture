...
| planning/application-profile/session-plan.md | v1.0 | 🟡 | **Tier 1 session plan (6 sessions, ~12–14 hrs).** Breaks Application Profile Tier 1 into six serial sessions: schema migration + data cleanup, `vw_application_profile` view, TypeScript interfaces, hooks, `ApplicationDetailDrawer` evolution, doc alignment + publish-assessment RPC reshape. Cross-references open-items matrix — bundles #86, #87, #94 Phase 1 schema into Session 1; #97 (optional #96) into Session 3. No merge-conflict risk (prerequisites #93 and #67 already on main). Tier 1 excludes Tier 2 concept tables (capabilities, data domains, tech debt items) and narrative generation pipeline. |
